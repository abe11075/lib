...
Library.Folder = "LynxGUI_Configs"
Library.SaveName = "lynx_config"

local function GetConfigPath()
	return Library.Folder .. "/" .. Library.SaveName .. ".json"
end
local CurrentConfig = {}
local DefaultConfig = {}
local isDirty = false
...
local ALIGN_CENTER = Enum.TextXAlignment.Center
...
local function new(class, props)
	...
	if props then
		local cornerVal = props.Corner
		local strokeVal = props.Stroke
		local strokeColorVal = props.StrokeColor
		
		if cornerVal then
			Instance.new("UICorner", inst).CornerRadius = UDim.new(0, cornerVal)
		end
		if strokeVal or strokeColorVal then
			local stroke = Instance.new("UIStroke", inst)
			stroke.Color = strokeColorVal or Color3.new(0, 0, 0)
			stroke.Thickness = strokeVal or 1
		end
		
		local padVal = props.Padding
		if type(padVal) == "table" then
			local pad = Instance.new("UIPadding", inst)
			pad.PaddingTop = UDim.new(0, padVal.Top or padVal[1] or 0)
			pad.PaddingBottom = UDim.new(0, padVal.Bottom or padVal[2] or 0)
			pad.PaddingLeft = UDim.new(0, padVal.Left or padVal[3] or 0)
			pad.PaddingRight = UDim.new(0, padVal.Right or padVal[4] or 0)
		end
		
		local layoutVal = props.ListLayout
		if layoutVal then
			local layout = Instance.new("UIListLayout", inst)
			layout.Padding = UDim.new(0, layoutVal.Padding or 0)
			layout.SortOrder = layoutVal.SortOrder or Enum.SortOrder.LayoutOrder
			if layoutVal.FillDirection then layout.FillDirection = layoutVal.FillDirection end
			if layoutVal.HorizontalAlignment then layout.HorizontalAlignment = layoutVal.HorizontalAlignment end
			if layoutVal.VerticalAlignment then layout.VerticalAlignment = layoutVal.VerticalAlignment end
		end
		...
		for k, v in pairs(props) do
			if k ~= "Font" and k ~= "FontFace" and k ~= "Parent" and k ~= "Corner" and k ~= "Stroke" and k ~= "StrokeColor" and k ~= "ListLayout" then
...
function Library:ApplyHover(instance, normalColor, hoverColor, propertyName)
	propertyName = propertyName or "BackgroundColor3"
	self:AddConnection(tostring(instance) .. "_hover_in", instance.MouseEnter:Connect(function()
		TweenService:Create(instance, TweenInfo.new(0.2), {[propertyName] = hoverColor}):Play()
	end))
	self:AddConnection(tostring(instance) .. "_hover_out", instance.MouseLeave:Connect(function()
		TweenService:Create(instance, TweenInfo.new(0.2), {[propertyName] = normalColor}):Play()
	end))
end

function Library:_createContainer(parent, height, zindex, corner)
	return new("Frame", {
		Parent = parent,
...
local function EnsureFolderExists()
	if (not isfolder) or (not isfolder(Library.Folder)) then
		if makefolder then pcall(makefolder, Library.Folder) end
	end
end
...
local function PrettyJSON(obj, indent)
	indent = indent or ""
	...
	if count == 0 then return isArray and "[]" or "{}" end
	local nextIndent = indent .. "  "
	local parts = {}
	if isArray then
		for i = 1, count do
			table.insert(parts, nextIndent .. PrettyJSON(obj[i], nextIndent))
		end
		return "[\n" .. table.concat(parts, ",\n") .. "\n" .. indent .. "]"
	else
		local keys = {}
		...
		for _, k in ipairs(keys) do
			table.insert(parts, nextIndent .. HttpService:JSONEncode(tostring(k)) .. ": " .. PrettyJSON(obj[k], nextIndent))
		end
		return "{\n" .. table.concat(parts, ",\n") .. "\n" .. indent .. "}"
	end
end
...
function Library.ConfigSystem.Save()
	local ok, err = pcall(function()
		EnsureFolderExists()
		local encoded = PrettyJSON(CurrentConfig)
		if writefile then writefile(GetConfigPath(), encoded) end
	end)
	if not ok and err then
		warn("[LynxGUI - Config Save Error]: " .. tostring(err))
	end
	return ok
end
function Library.ConfigSystem.Load()
	...
	local ok, err = pcall(function()
		...
		if isfile and isfile(path) then
			local raw = nil
			if readfile then raw = readfile(path) end
	...
	if not ok then
		if err then warn("[LynxGUI - Config Load Error]: " .. tostring(err)) end
		pcall(function()
			local path = GetConfigPath()
			if isfile and isfile(path) and readfile and writefile then
				pcall(writefile, path .. ".bak", readfile(path))
			end
		end)
		CurrentConfig = DeepCopy(DefaultConfig)
	end
...
function Library.ConfigSystem.Delete()
	local ok, err = pcall(function()
		...
		if isfile and isfile(path) then
			...
			elseif removefile then removefile(path) end
		end
	end)
	if not ok and err then
		warn("[LynxGUI - Config Delete Error]: " .. tostring(err))
	end
end
local function MarkDirty()
	if Library.AutoSaveEnabled == false then
		return
	end
	isDirty = true
	...
	Library._saveThread = task.delay(2, function()
		if not isDirty or Library.AutoSaveEnabled == false then
			Library._saveThread = nil
			return
		end
		local ok, err = pcall(function()
		...
		if not ok and err then
			warn("[LynxGUI - MarkDirty Error]: " .. tostring(err))
		end
		isDirty = false
		Library._saveThread = nil
	end)
...
function Library.GetConfigValue(key, default)
	return Library.ConfigSystem.Get(key, default)
end
function Library.SaveConfigValue(key, value)
	Library.ConfigSystem.Set(key, value)
	if Library.AutoSaveEnabled then
		MarkDirty()
	end
end
function Library.GetFullConfig()
	return CurrentConfig
end
Library.flags = {}
Library.pages = {}
...
WindowPrototype.__index = WindowPrototype
...
function WindowPrototype:AddTab(tabConfig)
	...
	local TabObject = setmetatable({}, TabPrototype)
	TabObject._page = page
	TabObject._library = self._library
	TabObject._sections = {}
	TabObject._tabName = tabName
...
function WindowPrototype:BuildDefaultConfigTab()
	local configTab = self:AddTab({ Name = "Config", Icon = "loop" })
	...
	autoSaveSection:AddToggle({
		...
		Callback = function(val)
			Library.AutoSaveEnabled = val
			self._library:MakeNotify({
				Title = "Auto Save",
				Description = val and "Auto Save enabled" or "Auto Save disabled",
				Delay = 2,
			})
		end,
	})
	...
	mgmtSection:AddButton({
		...
		Callback = function()
			...
			self._library:MakeNotify({
				Title = "Config",
				Description = ok and "Config saved successfully!" or "Failed to save config.",
	...
	mgmtSection:AddParagraph({
		Title = "⚠️ Attention",
		Content = "After resetting to default, settings like Toggles and Inputs will update in the UI immediately.\n\n"
			.. "However, for settings that affect your character, speed, or other active features — you may need to Rejoin / Respawn for the changes to fully apply.\n\n"
			.. "The config file is saved automatically every 2 seconds if Auto Save is active. Make sure Auto Save is ON before leaving the game so your settings aren't lost.",
	})
	...
	deleteBtnFrame = mgmtSection:AddButton({
		...
		Callback = function()
			if not deleteConfirm then
				...
				local btn = deleteBtnFrame:FindFirstChildWhichIsA("TextButton")
				if btn then
					btn.Text = "Delete Config File"
				end
				setBtnColor(deleteBtnFrame, colors.primary)
				Library.ConfigSystem.Delete()
				self._library:MakeNotify({
					Title = "Config",
					Description = "Config file has been deleted.",
					Color = Color3.fromRGB(220, 50, 50),
					Delay = 2,
				})
			end
...
function TabPrototype:AddSection(sectionTitle, isOpen)
	...
	local SectionObject = setmetatable({}, SectionPrototype)
	SectionObject._container = category
	SectionObject._library = self._library
	SectionObject._layoutOrder = 0
	SectionObject._tabName = self._tabName
	SectionObject._sectionTitle = sectionTitle
	SectionObject._sectionExpand = sectionExpand
	table.insert(self._sections, SectionObject)
	return SectionObject
end

function SectionPrototype:_getNextLayoutOrder()
	self._layoutOrder = self._layoutOrder + 1
	return self._layoutOrder
end

function SectionPrototype:_registerFeature(featureName, featureFrame, featureKind)
	...
	table.insert(lib._searchIndex, {
		...
		frame = featureFrame,
		pageName = self._tabName,
		sectionTitle = self._sectionTitle,
		kind = featureKind,
		expand = self._sectionExpand,
	})
end

function SectionPrototype:AddToggle(toggleConfig)
	...
	if frame then
		frame.LayoutOrder = self:_getNextLayoutOrder()
	end
	self:_registerFeature(title, frame, "Toggle")
...
function SectionPrototype:AddDropdown(dropdownConfig)
	...
	if isMulti then
		local frame =
			self._library:CreateMultiDropdown(self._container, title, nil, options, configPath, callback, uniqueId)
		if frame then
			frame.LayoutOrder = self:_getNextLayoutOrder()
		end
		self:_registerFeature(title, frame, "Dropdown")
		local dropdownObj = {
			_options = options,
	...
	local frame =
		self._library:CreateDropdown(self._container, title, nil, options, configPath, callback, uniqueId, default)
	if frame then
		frame.LayoutOrder = self:_getNextLayoutOrder()
	end
	self:_registerFeature(title, frame, "Dropdown")
...
function SectionPrototype:AddInput(inputConfig)
	...
	self:_registerFeature(title, frame, "Input")
	return {
		SetValue = function(self, val) end,
	}
end
function SectionPrototype:AddButton(buttonConfig)
	...
	self:_registerFeature(title, frame, "Button")
	return frame
end
function SectionPrototype:AddParagraph(paragraphConfig)
	...
	local frame = new("Frame", {
		...
		ZIndex = 7,
		LayoutOrder = self:_getNextLayoutOrder(),
	})
...
function Library:CreateWindow(config)
	...
	Library.Folder = "LynxGUI_Configs"
	Library.SaveName = config.SaveName or "lynx_config"
	...
	local discordTextW = game:GetService("TextService")
		:GetTextSize(discordText, fontSize.small, FONT_BOLD, Vector2.new(1000, 100)).X
...
function Library:CreateCategory(parent, title, startOpen)
	...
	local contentListLayout = new(
		"UIListLayout",
		{ Parent = contentContainer, Padding = UDim.new(0, 3), SortOrder = Enum.SortOrder.LayoutOrder }
	)
	local isOpen = startOpen
	arrow.Rotation = startOpen and 180 or 0
	local function updateCategoryHeight()
		if not categoryFrame or not categoryFrame.Parent then
			return
		end
...
function Library:_addCorner(parent, radius)
	return new("UICorner", {
		Parent = parent,
		CornerRadius = UDim.new(0, radius),
	})
end
function Library:_createBaseElement(parent, label, height, titleSize)
	local frame = new("Frame", {
		Parent = parent,
...
function Library:CreateToggle(parent, label, configPath, callback, disableSave, defaultValue)
	local frame = self:_createBaseElement(parent, label, 28, UDim2.new(1, -45, 1, 0))
	...
	self.flags[configPath or label] = on
	if self._initialized and callback then
...
function Library:_createBaseDropdown(
	...
	local titleText = title or (isMulti and "Multi Select" or "Dropdown")
	local dropdownFrame, dropdownTitle = self:_createBaseElement(parent, titleText, 28, UDim2.new(0.5, 0, 1, 0))
	dropdownFrame.Name = uniqueId or (isMulti and "MultiDropdown" or "Dropdown")
	...
	local function ensurePoolBuilt()
		...
		self:AddConnection(
			"dropdownScroll_" .. dropdownLayoutOrder,
			scrollSelect:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
				refreshVisible()
			end)
		)
		self:AddConnection(
			"dropdownResize_" .. dropdownLayoutOrder,
			scrollSelect:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
				if isOpen then
					refreshVisible()
				end
			end)
	...
	self:AddConnection(
		"dropdownOverlayClose_" .. dropdownLayoutOrder,
		self._dropdownOverlay:GetPropertyChangedSignal("Visible"):Connect(function()
	...
	if self._initialized and onSelect then
		task.spawn(function()
			if isMulti then
				onSelect(DropdownFunc.Value)
			else
...
function Library:CreateInput(parent, label, configPath, defaultValue, callback)
	local frame = self:_createBaseElement(parent, label, 28, UDim2.new(0.52, 0, 1, 0))
...
function Library:CreateButton(parent, label, callback)
	local btnFrame = self:_createContainer(parent, 28, 8, 5)
...
function Library:Initialize()
	...
	if self._pendingWindowObj then
		pcall(function()
			self._pendingWindowObj:BuildDefaultConfigTab()
		end)
...
function Library:MakeNotify(config)
	...
	while #self._activeNotifs >= 2 do
		local old = self._activeNotifs[1]
		table.remove(self._activeNotifs, 1)
		pcall(function()
	...
	if not self._notifContainer then
		self._notifContainer = new("Frame", {
			Parent = self._gui,
